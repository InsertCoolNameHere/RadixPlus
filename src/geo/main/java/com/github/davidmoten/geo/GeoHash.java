...
import geo.main.java.com.github.davidmoten.geo.util.Preconditions;
...
public final class GeoHash {
     ...
     */
    public static final int DEFAULT_MAX_HASHES = 2000;
    ...
    public static boolean hashContains(String hash, double lat, double lon) {
        ...
        return Math.abs(centre.getLat() - lat) <= heightDegrees(hash.length()) / 2
                && Math.abs(Position.to180(centre.getLon() - lon)) <= widthDegrees(hash.length()) / 2;
    }
...
//        Preconditions.checkArgument(length > 0, "length must be greater than zero");
    	//width and height of a geohash of precision=length
        ...
        //longitudinal difference between top left and bottom right points. 
        //In cases of odd shaped polygons, this is an error case
