...
public class dummyDataMatchTaxi {
	public static void main(String [] args) throws IOException{
		File dummyData = new File("/s/" + InetAddress.getLocalHost().getHostName() + "/a/tmp/dummy.csv");
...
//		long numLinesPerPlot = freeSpaceBytes/300/5693;
		long numLinesPerPlot = (long)(4294967296.0 * 2.5) / 120 / 1 / 5693;
...
//		System.out.println(((JSONArray)coords.get(0)).get(0));
		...
		Date time = new Date(System.currentTimeMillis()+(3600000*24));//3,600,000 ms = 1 hr
...
//					randGeno+= alpha.charAt(ThreadLocalRandom.current().nextInt(alpha.length()));
				...
				for (long i = 0; i < numLinesPerPlot; i++){//start super small to test database
					...
					record.append(ThreadLocalRandom.current().nextInt(0,1000000)+",");
					record.append(geno.replaceAll(",","-")+",");					
					record.append(ThreadLocalRandom.current().nextInt(1,69) + ",");
					record.append(formatter.format(time) + ",");
					record.append(ThreadLocalRandom.current().nextDouble()+",");
					record.append(ThreadLocalRandom.current().nextDouble()+",");
					record.append(randCoord[0] + "," + randCoord[1]);
...
//						record += "true";
					...
					bytesWritten += record.toString().getBytes().length +1;//add 1 to account for extra new line
